...
import random
import time
from aarsh import Image_recognition
...
class Login_Window:
    def __init__(self,root):
        ...
        # registerbutton
        registerbtn=Button(frame,text="Username; aarshit tripathi",command=self.rigister_window,font=("times new roman",10,"bold"),borderwidth=0,fg="white",bg="black",activeforeground="white",activebackground="black")
        registerbtn.place(x=15,y=320,width=160)

        # forgetpassbtn
        registerbtn=Button(frame,text="Password; ashu",command=self.forgot_password_window,font=("times new roman",10,"bold"),borderwidth=0,fg="white",bg="black",activeforeground="white",activebackground="black")
    ...
    def login(self):
        ...
        elif self.txtuser.get()=="aarshit tripathi" and self.txtpass.get()=="ashu":
            messagebox.showinfo("Success","Welcome to code with Ankit")
            self.new_window=Toplevel(self.root)
            self.app=Image_recognition(self.new_window)
        else:
            conn=mysql.connector.connect(host="localhost",user="root",password="saanjh143",database="mydata2")
